...
Linearize(DATA<TV>& data,const T dt,const T target_time,std::vector<Triplet<T>>& hessian_terms,std::vector<Triplet<T>>& force_terms,SparseMatrix<T>& constraint_terms,SparseMatrix<T>& constraint_forces,Matrix<T,Dynamic,1>& right_hand_side,Matrix<T,Dynamic,1>& constraint_rhs,bool stochastic)
{
    ...
    for(int s1=0;s1<rigid_data->structures.size();s1++){
        ...
        for(int s2 : intersector.candidates){
            ...
            T slack_distance=-.0025;
            T distance_condition=-.001;
            std::pair<int,T>& remembered=force_memory[CONSTRAINT(s1,s2)];
            LOG::cout<<"Constraint violation: "<<constraint_violation<<std::endl;
            if(constraint_violation<0){
                ...
                if(constraint_violation<2*slack_distance){
                    if(remembered.first!=call_count){remembered.second=0;}
                    if(std::get<0>(constant_force_memory[constraint])==call_count){
                        ...
                        remembered.second=std::get<3>(constant_force_memory[constraint]);//std::exp(1-1/(1-sqr(constraint_violation/std::get<2>(constant_force_memory[constraint])-1)));
                    }
                ...
                else if(remembered.first==call_count || std::get<0>(constant_force_memory[constraint])==call_count){ // exponential falloff
                    right_hand_side_force=0;
#if 1
                    if(std::get<0>(constant_force_memory[constraint])!=call_count){
                        std::get<1>(constant_force_memory[constraint])=constraint_violation;
                        std::get<2>(constant_force_memory[constraint])=remembered.second;
                        std::get<3>(constant_force_memory[constraint])=constraint_violation;
                    }
                    T characteristic_length=std::get<1>(constant_force_memory[constraint]);
                    T exponent=-1/(1-sqr(constraint_violation/characteristic_length-1));
                    right_hand_side_force=std::get<2>(constant_force_memory[constraint])*std::exp(1+exponent);
                    std::get<4>(constant_force_memory[constraint])=constraint_violation-std::get<3>(constant_force_memory[constraint]);
                    constant_forces.push_back(CONSTRAINT(s1,s2));
                    LOG::cout<<"Force between "<<s1<<" and "<<s2<<" with force: "<<std::get<2>(constant_force_memory[constraint])<<" but also "<<right_hand_side_force<<" characteristic length "<<characteristic_length<<std::endl;
