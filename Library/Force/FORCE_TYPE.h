...
public:
    ...
    virtual void Pack_Forces(std::shared_ptr<FORCE_REFERENCE<T>> force_information) const{force_information->value=stored_forces;};
    virtual void Unpack_Forces(std::shared_ptr<FORCE_REFERENCE<T>> force_information){stored_forces=force_information->value;};
    virtual void Increment_Forces(std::shared_ptr<FORCE_REFERENCE<T>> force_information,T ratio){stored_forces+=ratio*force_information->value;};
