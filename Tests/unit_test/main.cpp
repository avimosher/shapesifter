...
TEST_CASE("Derivatives","[derivatives]"){
    ...
    SECTION("dRotatedOffset_dSpin"){
        ...
        for(int i=0;i<tests;i++){
            ...
            REQUIRE((actual_offset-estimated_offset).norm()<epsilon);
        }
    }
        ...
        for(int i=0;i<tests;i++){
            ...
            for(int s1=0;s1<2;s1++){
                ...
                for(int s2=0;s2<2;s2++){
                    ...
                    REQUIRE((actual_direction-estimated_direction).norm()<2*(derivative*delta).norm());
                }
            }
        }
    }
